...
@Component({
    selector: 'app-daily-ayat',
    templateUrl: './daily-ayat.component.html',
    styleUrl: './daily-ayat.component.scss',
})
export class DailyAyatComponent {
    dailyAyat: any;
    ...
    GetVerseOfTheDay() {
        this.sharedService.getdailyAyat().subscribe((res: any) => {
            if (res?.result?.success) {
                this.dailyAyat = res?.result?.data;
            }
        });
    }
    ...
    async shareContent() {
        ...
        const shareIcon =
            this.cardSection.nativeElement.querySelector('.share-icon');
        if (shareIcon) {
        ...
        try {
            // Capture the canvas
            const canvas = await html2canvas(this.cardSection.nativeElement, {
                useCORS: true,
                allowTaint: true,
            ...
            if (shareIcon) {
                shareIcon.style.visibility = 'visible'; // restore quickly
            }
            ...
            const file = new File([blob], 'quran-verse.png', {
                type: 'image/png',
            });

            const shareData: ShareData = {
                title: this.dailyAyat.tema,
                text: `${this.dailyAyat.arabicVerse}\n${this.dailyAyat.indonesiaVerse}\n${this.dailyAyat.latinVerse}\n${this.dailyAyat.surah} ${this.dailyAyat.lastNumber}`,
                files: [file],
            };
            ...
            const fallbackText = `${this.dailyAyat.tema}\n${this.dailyAyat.arabicVerse}\n${this.dailyAyat.indonesiaVerse}\n${this.dailyAyat.latinVerse}\n${this.dailyAyat.surah} ${this.dailyAyat.lastNumber}`;
